...
{
    whcontrol.sph_c = getEEPROMFloatValue(SPH_ADDRESS, DEFAULT_SPH);
    whcontrol.spl_c = getEEPROMFloatValue(SPL_ADDRESS, DEFAULT_SPL);
    whcontrol.pfl_lpm = getEEPROMFloatValue(PFL_ADDRESS, DEFAULT_PFL);
    whcontrol.unpausedelayms = getEEPROMIntValue(FUDL_ADDRESS, DEFAULT_FUDL);
    whcontrol.pausedelayms = getEEPROMIntValue(FPDL_ADDRESS, DEFAULT_FPDL);

    // save the settings to EEPROM now in case the defaults had to be used.
    // If they are the same no write will happen
